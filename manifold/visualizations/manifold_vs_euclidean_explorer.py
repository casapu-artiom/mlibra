...
All the numeric machinery is reused from ``graph_visualization_utils.py``
...
-------------------------------
  ...
  * TRAINED  -- ``--manifold-run <dir>`` / ``--euclidean-run <dir>`` read that
                run's config.json and its LEARNED lengthscale/outputscale/noise
                (verbatim reproduction -- see below). Two checkpoint layouts:
                  whole-brain  <run>/model.pth
                  per-lipid    <run>/checkpoints/batch_*.pt  -- REQUIRES
                               ``--run-lipid <name>`` (each lipid has its own
                               learned outputscale/noise); omit it once to see
                               the available lipid names in the error.
                Both ``--*-run`` flags share ``--run-lipid``, so passing the
                same name to both compares euclidean vs manifold AT that lipid.
...
Headless
--------
  python manifold/viz/manifold_vs_euclidean_explorer.py --self-test [--compute-if-missing]
runs the entire numeric pipeline and prints the report, no display needed.

Launch the GUI
--------------
  python manifold/viz/manifold_vs_euclidean_explorer.py \
...
Reproducing a TRAINED model exactly
------------------------------------
  ``--manifold-run``/``--euclidean-run`` populate this automatically. To set it
  by hand instead, --euclidean/--manifold (and the GUI's noise sliders) take a
  ``noise=`` key: an ABSOLUTE observation-noise variance (z-units²), not the
  default relative alpha grid-search. Set it to a run's LEARNED noise (alongside
  its learned lengthscale/outputscale) to see the R² and covariance heatmap that
  model actually produces, instead of this tool's own auto-tuned best guess:
      --manifold ...,outputscale=2.3,lengthscale=1.8,noise=0.31
  ``noise=0`` (default) keeps the old auto-tune-per-lipid behaviour.

  Example, reproducing a per-lipid euclidean-ARD run at one lipid:
      --euclidean-run /home/casap/mlibra/output/per_lipid/FOLD-2-euclidean-ard-matern-... \
...
import graph_visualization_utils as G
...
# manifold/download_bg_atlas.py --max-depth: 175/476 regions, root down to 0.7%.
...
def analyze(pool: CellPool, eu: KernelSpec, man: KernelSpec, allow_solve=False,
           highlight_lipid=None, chosen_lipid=None):
    """Everything the notebook computes, for one (euclidean, manifold) pair.

    ``highlight_lipid`` -- e.g. the lipid a loaded --*-run was actually trained
    on (--run-lipid) -- gets its OWN held-out R²/MSE row pulled out of the same
    per_lipid_r2 fit (no refit), shown ahead of the all-173-lipid summary.

    ``chosen_lipid`` -- whichever lipid is CURRENTLY picked in the GUI's
    "lipid (true/pred/error)" dropdown, which can be browsed to anything and
    generally differs from `highlight_lipid` (that one stays fixed to the
    loaded run; see the "pick once" design). Gets its own block too, skipped
    if it's the same lipid as `highlight_lipid` (avoids a duplicate block)."""
    specs = [eu, man]
    ...
    lipid_summary = _lipid_summary(pool, pl, specs, highlight_lipid)
    chosen_summary = (_lipid_summary(pool, pl, specs, chosen_lipid)
                      if chosen_lipid and chosen_lipid != highlight_lipid else None)
    ...
    return dict(health=health, prize=pz, summary=summary, lipid_summary=lipid_summary,
                chosen_summary=chosen_summary, compare=cmp, border=border, inside=inside,
                per_lipid=pl, eu=eu, man=man, atlas=Path(pool.annotations).stem)


def _lipid_summary(pool, pl, specs, lipid):
    """Held-out R²/MSE for ONE specific lipid, pulled straight out of the
    already-fitted per_lipid_r2 result (no refit) -- e.g. the lipid a loaded
    --*-run was actually trained on. Returns None if `lipid` wasn't given or
    isn't in this pool (silently skipped by the report)."""
    if not lipid or lipid not in pl["lipids"]:
        return None
    j = pl["lipids"].index(lipid)
    Z = pool.Z.astype(np.float64); te = pl["test"]
    truth = Z[te, j]
    rows = {}
    for s in specs:
        pred = pl["preds"][s.name][:, j]
        rows[s.name] = dict(r2=float(pl["r2"][s.name][j]),
                            mse=float(((truth - pred) ** 2).mean()))
    return dict(lipid=lipid, n_test=len(te), rows=rows)
...
def format_report(res: dict) -> str:
    ...
    L.append("")
    ls = res.get("lipid_summary")
    if ls:
        L.append(f"TRAINED LIPID — {ls['lipid']!r}  ({ls['n_test']} held-out test cells)")
        L.append(f"   {'model':<38}{'R²':>9}{'MSE':>8}")
        for nm, r in ls["rows"].items():
            L.append(f"   {nm[:37]:<38}{r['r2']:>9.3f}{r['mse']:>8.3f}")
        L.append("   (this is the ONE lipid the loaded run was trained on -- the SUMMARY "
                 "below is all 173 lipids)")
        L.append("")
    cs = res.get("chosen_summary")
    if cs:
        L.append(f"SELECTED LIPID — {cs['lipid']!r}  ({cs['n_test']} held-out test cells)")
        L.append(f"   {'model':<38}{'R²':>9}{'MSE':>8}")
        for nm, r in cs["rows"].items():
            L.append(f"   {nm[:37]:<38}{r['r2']:>9.3f}{r['mse']:>8.3f}")
        L.append("   (currently picked in the 'lipid (true/pred/error)' dropdown -- the kernels "
                 "themselves are NOT retuned to it, this is just their score on it)")
        L.append("")
...
class MaldiSlices:
    ...
    def __init__(self, pool, sample, gap_mm=1.0):
        import pandas as pd
        import manifold.viz.maldi_kernel_explorer as mke
...
def region_rgba(pool):
    ...
    from manifold.viz.maldi_kernel_explorer import region_color_table
...
def predict_field(pool, spec, lipid, slice_nodes, allow_solve=False, n_train=1500, seed=0):
    ...
    if spec.noise:                     # FIXED absolute noise -- reproduce a trained model
        alpha_abs = spec.noise
    else:
        ...
        for al in [1e-2, 3e-2, 1e-1, 3e-1, 1.0]:
            ...
            if r2 > best[0]: best = (r2, al * d)
        alpha_abs = best[1]
    sol = np.linalg.solve(Ktt + alpha_abs * np.eye(len(tr)), ytr)
...
# ===========================================================================
def _heat_colors(vals, gamma=0.45, cmap="inferno", fade=True, alpha_floor=0.15, vlim=None):
    """Strong sequential heatmap: gamma-boost the high end, and (fade=True)
    modulate OPACITY by the value so low-covariance points recede.

    ``vlim=None`` (default): robust-normalise against THIS frame's own 1st/99.5th
    percentiles -- always fills the colour range, so a uniform rescale of ``vals``
    (e.g. the outputscale) is invisible.
    ``vlim=<float>``: fixed absolute scale, ``lo=0, hi=vlim`` -- a uniform rescale
    of ``vals`` now visibly brightens/dims/saturates the map."""
    import matplotlib
...
def launch(pool, eu0, man0, allow_solve, sample="ReferenceAtlas", run_lipid=None):
    import napari
    ...
    # default the true/pred/error dropdown to the lipid a loaded --*-run was
    # trained on (--run-lipid), if it's a valid lipid in this pool; the SUMMARY
    # panel's "TRAINED LIPID" block always refers back to `run_lipid` itself
    # (unresolved), regardless of what the dropdown gets changed to later.
    default_lipid = run_lipid if run_lipid in pool.lipids else pool.lipids[0]
    state = dict(eu=eu0, man=man0, which="manifold", slices=slices, ptsz=0.05,
                 section=slices.sections[len(slices.sections) // 2],
                 lipid=default_lipid, test_node=None, cur_pts=None, cur_nodes=None,
                 raw_cov=False)
    ...
    def repaint_heatmap():
        try:
            ...
            return
        vlim = None
        if state.get("raw_cov"):
            # Fixed absolute scale: percentile of the UNIT-outputscale kernel
            # (kv / outputscale cancels the current outputscale exactly, so this
            # reference depends only on kernel SHAPE -- nu/lengthscale/graph --
            # not on the outputscale slider). Normalising kv against this FIXED
            # number, instead of kv's own percentile, is what makes moving the
            # outputscale slider visibly brighten/dim/saturate the heatmap.
            os_ = max(_spec().outputscale, 1e-12)
            vlim = float(np.nanpercentile(kv / os_, 99.5))
        cov_layer.face_color = _heat_colors(kv, vlim=vlim)  # colour + opacity fade
        cov_layer.refresh()
    ...
    def refit():
        ...
        try:
            res = analyze(pool, state["eu"], state["man"], allow_solve=allow_solve,
                          highlight_lipid=run_lipid, chosen_lipid=state["lipid"])
        except FileNotFoundError as e:
            report.setPlainText("EIGENPAIRS NOT CACHED\n\n" + str(e) +
                                "\n\nTick 'compute if missing' + Refit.")
            return
    ...
    def _frange(value, lo, hi, step, pad=10.0):
        """Widen (lo, hi) and shrink `step` if needed so magicgui's
        FloatSpinBox/SpinBox neither rejects `value` at construction time nor
        silently rounds it off. Two independent magicgui/Qt behaviours bite a
        loaded --*-run's learned outputscale/lengthscale/noise, which can be
        far outside the range these sliders assume for hand-tuning:
          1. min/max reject any value outside them (e.g. outputscale=0.002
             against a 0.01-100 hand-tuning range) -> raises at construction.
          2. the widget's DISPLAY DECIMALS are derived from step's magnitude
             (ceil(abs(log10(step)))), not from value's -- so even after (1) is
             fixed, a step like value/10 can still round 0.002128 down to 0.
        Fix: widen the range to always contain `value`, and force a small
        FIXED step (not value-relative) so decimals is always generous enough
        to round-trip any loaded value exactly, regardless of its magnitude."""
        if value is None:
            return dict(min=lo, max=hi, step=step)
        lo2 = min(lo, value / pad) if value > 0 else min(lo, value)
        hi2 = max(hi, value * pad) if value > 0 else max(hi, value)
        step2 = min(step, 1e-6) if value else step
        return dict(min=lo2, max=hi2, step=step2)

    def _irange(value, lo, hi, step, pad=10.0):
        """Integer-SpinBox counterpart of `_frange` -- only widens min/max (no
        decimals/step-precision issue for an int SpinBox; passing a fractional
        step there silently truncates to 0 and kills the +/- arrows)."""
        if value is None:
            return dict(min=lo, max=hi, step=step)
        lo2 = min(lo, int(value / pad)) if value > 0 else min(lo, value)
        hi2 = max(hi, int(value * pad)) if value > 0 else max(hi, value)
        return dict(min=lo2, max=hi2, step=step)
    ...
    @magicgui(
        ...
        raw_cov={"label": "heatmap: raw covariance (abs)"},
        ...
        eu_ls={"label": "Euclid ℓ (mm)", "widget_type": "FloatSpinBox",
              **_frange(eu0.lengthscale, 0.1, 20, 0.1)},
        eu_os={"label": "Euclid outputscale", "widget_type": "FloatSpinBox",
              **_frange(eu0.outputscale, 0.01, 100, 0.1)},
        eu_border={"label": "Euclid border down-weight", "widget_type": "FloatSpinBox", "min": 0, "max": 1, "step": 0.05},
        eu_noise={"label": "Euclid noise (0=auto-tune)", "widget_type": "FloatSpinBox",
                 **_frange(eu0.noise, 0, 10, 0.01)},
        man_method={"label": "Manifold graph", "choices": ["faiss", "faiss_atlas_weighted"]},
        man_infl={"label": "cross-region ×", "widget_type": "FloatSpinBox",
                 **_frange(man0.inflation, 1, 500, 1)},
        man_prune={"label": "prune fraction", "widget_type": "FloatSpinBox", "min": 0, "max": 1, "step": 0.05},
        man_denoise={"label": "denoise passes", "widget_type": "SpinBox",
                    **_irange(man0.denoise, 0, 5, 1, pad=4)},
        man_modes={"label": "num modes", "widget_type": "SpinBox",
                  **_irange(man0.num_modes, 20, 6000, 100)},
        man_bw={"label": "graph bandwidth", "widget_type": "FloatSpinBox",
               **_frange(man0.bandwidth, 0.01, 1, 0.01)},
        man_nu={"label": "Manifold ν", "choices": [1, 2, 3]},
        man_ls={"label": "Manifold ℓ", "widget_type": "FloatSpinBox",
               **_frange(man0.lengthscale, 0.1, 20, 0.1)},
        man_os={"label": "Manifold outputscale", "widget_type": "FloatSpinBox",
               **_frange(man0.outputscale, 0.01, 100, 0.1)},
        man_noise={"label": "Manifold noise (0=auto-tune)", "widget_type": "FloatSpinBox",
                  **_frange(man0.noise, 0, 10, 0.01)},
        compute={"label": "compute eigvecs if missing"},
    )
    def controls(atlas=cur_atlas, show="manifold", raw_cov=False, lipid=default_lipid,
                 ...
                 eu_border=eu0.border_downweight, eu_noise=eu0.noise,
        ...
        state["eu"] = KernelSpec.euclidean(nu=eu_nu, lengthscale=eu_ls, outputscale=eu_os,
                                           border_downweight=eu_border, noise=eu_noise)
        ...
        state["man"] = replace(man0, knn_method=man_method, inflation=man_infl, prune=man_prune,
                               ...
                               nu=man_nu, lengthscale=man_ls, outputscale=man_os, noise=man_noise,
...
def main():
    ...
    ap.add_argument("--manifold-run", default=None,
                    help="a trained run dir -> use its config + LEARNED lengthscale/outputscale/"
                         "noise for the manifold kernel (verbatim reproduction; see --run-lipid "
                         "for per-lipid runs). Overrides --manifold.")
    ap.add_argument("--euclidean-run", default=None,
                    help="same as --manifold-run, for the euclidean kernel. Overrides --euclidean.")
    ap.add_argument("--run-lipid", default=None,
                    help="REQUIRED by --manifold-run/--euclidean-run when the run is a per-lipid "
                         "checkpoint (checkpoints/batch_*.pt) -- each lipid has its own learned "
                         "outputscale/noise, so one must be picked. Omit --run-lipid once to see "
                         "the list of available lipid names in the error. Shared by both --*-run "
                         "flags (pass the same lipid to both to compare them on it).")
    ...
    eu = (KernelSpec.euclidean_from_run(a.euclidean_run, lipid=a.run_lipid) if a.euclidean_run
          else euclidean_spec(parse_kv(a.euclidean)))
    man = (KernelSpec.from_run(a.manifold_run, lipid=a.run_lipid) if a.manifold_run
           else manifold_spec(parse_kv(a.manifold)))

    if a.self_test:
        try:
            res = analyze(pool, eu, man, allow_solve=a.compute_if_missing,
                          highlight_lipid=a.run_lipid)
        except FileNotFoundError as e:
            print(str(e))
    ...
    launch(pool, eu, man, allow_solve=a.compute_if_missing, sample=a.sample, run_lipid=a.run_lipid)
