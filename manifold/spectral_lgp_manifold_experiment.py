"""Weight-space (spectral) Riemann manifold MALDI experiment.

The spectral twin of ``lgp_manifold_experiment.py``: identical graph / Laplacian
/ eigenpair pipeline (same caches, same knobs), but the latent GP is the
weight-space ``SpectralLatentGP`` (a diagonal variational posterior over the
manifold spectrum) instead of the inducing-point ``LatentRiemannGP``. Because
the spectral GP parametrizes the posterior directly in the eigenbasis there are
NO inducing points — so all the inducing-source / blend / snap machinery of the
inducing runner is gone here. The MLP decoder (``SpectralManifoldLGP``) is the
same architecture as ``ManifoldLGP`` so the two runners differ only in the GP.

Pair this with ``lgp_manifold_experiment.py`` (inducing-point baseline) for the
inducing-vs-spectral comparison, and with ``lgp_experiment.py`` (Euclidean
Matern) for the kernel comparison.
"""
...
from lgp_manifold import SpectralLatentGP, SpectralManifoldLGP
...
def parse_args():
    ...
    parser = ArgumentParser(description="Run MALDI experiment with l3di (spectral Riemann).")
    ...
    parser.add_argument("--num-inducing", dest="num_inducing", type=int, default=0,
                        help="VESTIGIAL for the spectral GP (it has no inducing points). "
                             "Only read by the shared MaldiConfig; the value is unused by "
                             "this runner. Kept so MaldiConfig.from_args doesn't KeyError.")
...
def setup_experiment(args):
    ...
    # The spectral GP has NO inducing points, so we skip all inducing selection
    # and only need the (global) coordinate normalization the graph nodes are
    # scaled by — taken straight from the reference image, exactly as the
    # inducing runner's --inducing-from-maldi-nodes path does.
    ...
    # 6. Riemann Matern kernel, fully formed at construction. The spectral GP
    # reads this kernel's eigenbasis (raw_eigenvectors) and spectral_density
    # directly — there is a SINGLE kernel (no per-task lengthscale wrapper and no
    # product-ARD ScaleKernel composition; those are inducing-path features).
    ...
    # 7. Weight-space spectral GP over the manifold spectrum, then end-to-end
    # spectral Manifold LGP. No inducing points: the variational posterior lives
    # directly on the num_modes eigen-coefficients.
    logging.info(f"Building SpectralLatentGP over {num_modes} manifold modes...")
...
if __name__ == "__main__":
    ...
    logging.info("Starting MALDI experiment with spectral Riemann Manifold")
    args = parse_args()
