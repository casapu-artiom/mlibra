...
Usage
-----
    python manifold/model_hyperparams.py /workspace/output/<EXP_NAME>
    python manifold/model_hyperparams.py /workspace/output --glob         # detail, every run
    python manifold/model_hyperparams.py /workspace/output --table        # compact sweep table
    python manifold/model_hyperparams.py /workspace/output/per_lipid --table   # one row per lipid
    python manifold/model_hyperparams.py <run> --raw                      # also show raw tensors
"""
...
def _is_mean_param(k: str, v) -> bool:
    """The GP mean function's learned params (ConstantMean.constant, LinearMean
    .weights/.bias). Not raw_/constrained, so include them as-is."""
    return torch.is_tensor(v) and "mean_module" in k


def _table_params(sd: dict, bounds: dict):
    """Yield (key, value) for every table-worthy parameter: constrained kernel/
    likelihood hyperparameters (transformed) and the mean-function params (raw)."""
    for k, v in sd.items():
        if _is_param(k, v):
            yield k, transform(v, bounds.get(k))[0]
        elif _is_mean_param(k, v):
            yield k, v.float()


def _put_vector(row: dict, name: str, val: torch.Tensor, expand_max: int) -> None:
    """Scalar -> one column; a SHORT vector (<= expand_max) -> one column per element
    (name.0, name.1, ...) PLUS a name[mean] column; a longer vector -> name[mean] only."""
    val = val.flatten()
    if val.numel() == 1:
        row[name] = round(float(val.item()), 6)
    elif val.numel() <= 4096:
        if val.numel() <= expand_max:
...
def extract_rows(path: Path, expand_max: int = 8) -> list[dict]:
    ...
    if not per_lipid:
        row = dict(base)
        # Kernel/likelihood hyperparameters AND the mean-function params. A short
        # vector (e.g. per-latent-GP outputscale, LinearMean weights) is expanded
        # to one column per element plus a [mean] column.
        for k, val in _table_params(sd, bounds):
            _put_vector(row, clean_name(k), val, expand_max)
        return [row]
    ...
    for i in task_range:
        ...
        for k, tval in _table_params(sd, bounds):
            ...
            if m:                                     # per-task kernel param
                ...
                elif n_tasks and val.numel() == n_tasks:  # e.g. per-task mean bias
