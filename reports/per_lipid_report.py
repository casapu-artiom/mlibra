...
parcelgp runs
-------------
Runs from ``local_run/run_parcel_per_lipid.sh`` are ordinary per-lipid runs with an extra
...
Usage
-----
    python reports/per_lipid_report.py /path/to/experiment_batch_14
    python reports/per_lipid_report.py out_a out_b out_c          # multiple roots pooled
    python reports/per_lipid_report.py /path/to/out --csv-dir ./report_out
    python reports/per_lipid_report.py /path/to/out --sort test_r2 --metric test_r2
    # compare the per-lipid GPs against the whole-brain LGP/manifold models:
    python reports/per_lipid_report.py /path/to/per_lipid_out \
...
def derive_parcel(run_dir: Path, config: dict | None) -> str:
    ...
    A parcelgp run (``local_run/run_parcel_per_lipid.sh`` -> ``--parcel-field``) wraps the
...
# runners (local_run/run_lgp.sh, local_run/run_parcel_lgp.sh) put the splits-file stem
