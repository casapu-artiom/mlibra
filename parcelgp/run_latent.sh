...
#
#   ./parcelgp/run_latent.sh                    # baseline + parcel, back to back
#   MODE=parcel   ./parcelgp/run_latent.sh
#   MODE=baseline ./parcelgp/run_latent.sh
#
# Runs parcelgp/lgp_parcel_experiment.py, which is maldi/lgp_experiment.py with
...
# ---- model / training (names mirror maldi/run_final.sh) ----
: "${LATENT_DIM:=5}"
...
python -m parcelgp.build --reference-file "$REFERENCE_FILE" \
...
run_baseline() {
    ...
    python -m parcelgp.lgp_parcel_experiment "${common[@]}"
}
run_parcel() {
    ...
    python -m parcelgp.lgp_parcel_experiment "${common[@]}" "${extra[@]}"
}
