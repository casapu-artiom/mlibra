...
from patched_experiment import MaldiPatchedExperiment
...
def setup_experiment(args):
    ...
    # 3. Extract Inducing Points (Anchor nodes for the Latent GP)
    logging.info(f"Locking {config.num_inducing} anchor nodes on the graph...")
    # indices = torch.randperm(train_x.size(0))[:config.num_inducing]
    # inducing_points = train_x[indices]
    #indices = torch.randperm(reference_nodes.size(0))[:config.num_inducing]
    #inducing_points = reference_nodes[indices]
    ...
    manifold_kernel = RiemannMaternKernel(
        ...
        x=reference_nodes, 
        nearest_neighbors=5,
        num_modes=200,      # Adjust based on tissue complexity (200-500 is typical)
    ...
    lgp_model = ManifoldLGP(
        ...
        n_neurons=[100, 100],
        dropout=[0.1, 0.1],
        activation='relu',  # Switched to SiLU as it generally decodes spatial fields smoother than ReLU
