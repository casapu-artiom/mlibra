...
#
# Local test (no sudo):   PREFIX=$HOME/opt bash slepc/build_slepc_petsc.sh
# Docker (as root):       PREFIX=/opt      bash slepc/build_slepc_petsc.sh
# Without MUMPS (fast):   WITH_MUMPS=0     bash slepc/build_slepc_petsc.sh
#
