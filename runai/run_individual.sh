#!/usr/bin/env sh
# ============================================================================
# Local single-run fork of maldi/run_manifold.sh.
#
# "individual" == one config, run right here (SSH into the pod and execute this).
# A run_batch.sh sweeping configs will live alongside it later.
#
# Paths are wired for the RunPod layout:
#   data  (annotations, maindata, reference, ...) -> /workspace/maldi_data
#   output (checkpoints, eigenvectors, renders)   -> /workspace/output
# The code repo is located from THIS script's path (parent of runai/), so it
# works wherever the repo is cloned. Every knob is still overridable via env.
# ============================================================================

# Repo root = parent of the dir holding this script (…/<repo>/runai/run_individual.sh).
...
: "${SLICES_DATASET_FILE:=${SRC_PATH}/maldi/data/splits/fold_1.json}"
...
mkdir -p "$OUTPUT_DIR" "$EIGENVECTOR_DIR"
cd $SRC_PATH
#pip install -e .
