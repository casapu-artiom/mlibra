...
class Explorer:
    ...
    def _build_controls(self):
        ...
        self.compute_btn.clicked.connect(self.on_compute_highlight)
    ...
    def _rebuild_all_layers(self, cfg, res):
        ...
        self._euc_ell = 5.0 * (auto_point_size(X) / 1.3)  # ~5 NN spacings
    ...
    def on_compute_highlight(self):
        ...
        man = kv / (kv[i] + 1e-12)
        self.dyn["man"].face_color = colormap_rgba(man, "viridis", 0, 1)
        euc_d = np.linalg.norm(self._X_kern - self._X_kern[i], axis=1)
        self.dyn["euc"].face_color = colormap_rgba(
            np.exp(-0.5 * (euc_d / self._euc_ell) ** 2), "viridis", 0, 1)
        ...
        self._status(f"Highlight computed at node {i}. Toggle 'manifold kernel: "
                     f"atlas (src)' vs 'euclidean kernel (src)' to compare the fold.")
